#! /bin/bash
# Build 20140607
# < LNMPA [Apache/Nginx|MYSQL/MariaDB|PHP] 安装脚本 >
#
...
#
# 作者：MTimer 此 LNMPA 经过实战, 灵感来自 NewEraCracker For Windows
#
# 支持32bit 64bit的 CentOS/RHEL 系 5 或 6
# 支持选择 Apache/MYSQL/MariaDB/PHP 版本
# 如果一路回车，您将默认安装 Nginx + MYSQL 5.6 + PHP 5.3
#
...
#│	如果有问题请直接 Email: mtimercms@hotmail.com  附上安装记录 lnmpa_install.log
#│	时光人官网: http://www.mtimer.cn
#│
#│	因为借鉴了很多文章和脚本，需要感谢很多人
#│	感谢名单详见: http://www.mtimer.cn
#└────────────────────────────────────────────────
#┘
...
# 为什么这样做？有得有失，可以解决磁盘IO慢的问题，但是/tmp目录下的文件重启后就消失，但是此LNMPA脚本可以应付。
a=$(date +%s)
mv /tmp /tmp-${a}
ln -s /dev/shm /tmp
...
# 如果已经安装了 时光人系统 你有10秒钟的时间取消安装，否则覆盖。
if [ -e /mtimer/www/${mtimer_dir} ]; then
	...
	# crontab -e
	# 如果是 Apache 添加
	# 添加00 00 * * * /bin/bash /mtimer/server/httpd/crons/split_httpd_log.sh
	# 如果是 Nginx 添加
	...
	# 安装完成后屏幕会显示MYSQL,VSFTP的账号,也可以查看cat account.log查看
	echo "";
	cat <<-EOF
	+-------------------------------------------------+
